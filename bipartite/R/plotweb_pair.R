plotweb_pair <- function(web1,
  ...
  if (!is.null(add_higher_abundances)) {
    ...
    if (!is.null(add_c_abun_names_1)) {
      if (length(add_c_abun_names_1) == nc) {
        ...
        add_c_abundances_1 <- append(add_c_abundances_1, missing_values)
        add_c_abundances_1 <- add_c_abundances_1[c_names]
    ...
    if (!is.null(add_c_abun_names_2)) {
      if (length(add_c_abun_names_2) == nc) {
        ...
        missing_names <- setdiff(c_names, add_c_abun_names_2)
        missing_values <- rep(0, nc - nc_2)
        names(missing_values) <- missing_names
        add_c_abundances_2 <- append(add_c_abundances_2, missing_values)
        add_c_abundances_2 <- add_c_abundances_2[c_names]
      } else {
      ...
      } else if (length(add_c_abundances_2) == 0) {
        add_c_abundances_2 <- rep(0, nc)
      } else {
  ...
  max_prop_size <- max(sum(c_prop_sizes_max),
                       sum(r_prop_sizes_1),
                       sum(r_prop_sizes_2))
  ...
  if (!is.null(add_higher_abundances)) {
    ...
    c_xl_1 <- rep(0, nc)
    c_xr_1 <- rep(0, nc)
    c_xl_1[c(TRUE, FALSE)] <- c_xl + 0.5 * (c_prop_sizes - colSums(matrix(c_prop_sizes_1, nrow = 2)))
    c_xr_1[c(TRUE, FALSE)] <- c_xl_1[c(TRUE, FALSE)] + c_prop_sizes_1[c(TRUE, FALSE)]
    c_xl_1[c(FALSE, TRUE)] <- c_xr_1[c(TRUE, FALSE)]
    c_xr_1[c(FALSE, TRUE)] <- c_xr_1[c(TRUE, FALSE)] + c_prop_sizes_1[c(FALSE, TRUE)]

    c_xl_2 <- rep(0, 2 * nc)
    c_xr_2 <- rep(0, 2 * nc)
    c_xl_2[c(TRUE, FALSE)] <- c_xl + 0.5 * (c_prop_sizes - colSums(matrix(c_prop_sizes_2, nrow = 2)))
    c_xr_2[c(TRUE, FALSE)] <- c_xl_2[c(TRUE, FALSE)] + c_prop_sizes_2[c(TRUE, FALSE)]
  ...
  if (horizontal) {
    ...
    text(-0.01, r_tx_1, r_names_1, adj = c(1, 0.5), xpd = TRUE,
         cex = text_size, srt = srt, col = lower_text_color)
    text(1 + 0.5 * grconvertX(c_m_t_width + r_m_t_width_1 + 0.3,
                              from = "inches"),
    ...
    text(r_tx_1, -0.01, r_names_1, adj = c(1, 0.5), xpd = TRUE,
         cex = text_size, srt = srt + 90, col = lower_text_color)
    text(c_tx, 1 + 0.5 * grconvertY(c_m_t_width + r_m_t_width_1 + 0.3,
                                    from = "inches"),
  ...
  if (horizontal) {
    ...
    text(1.01, r_tx_2, r_names_2, adj = c(0, 0.5), cex = text_size,
         xpd = TRUE, srt = srt, col = lower_text_color)
  } else {
    ...
    text(r_tx_2, 1.01, r_names_2, adj = c(0, 0.5), cex = text_size,
         xpd = TRUE, srt = srt + 90, col = lower_text_color)
  }
  ...
  if (!is.null(add_higher_abundances) && !is.null(add_lower_abundances)) {
    ...
    web.df_2 <- data.frame(row = rep(1:nr_2, nc),
                           col = rep(1:nc, each = nr_2),
                           weight = c(web2))
  }
